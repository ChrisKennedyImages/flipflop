...
with st.sidebar:
    ...
    key_input = st.text_input(
        ...
        type="password",
        placeholder="sk-ant-...",
        help="Your key is stored only in this browser session — never saved to disk.",
        label_visibility="collapsed"
    )
    if key_input != st.session_state.api_key:
        st.session_state.api_key = key_input

    st.caption("🔒 Key lives in session memory only — never written to disk.")
    st.markdown("---")
    st.markdown("**Model:** `claude-sonnet-4-20250514`")
    st.markdown("**Vision:** ✅ Enabled")
